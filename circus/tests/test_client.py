...
class TestSigHandler(TestCircus):
    ...
    def test_handler(self):
        ...
        self.assertEquals(call("list"), 'test')
        self.assertEquals(call("list test"), '10')
        self.assertEquals(call("incr test"), '2')
        self.assertEquals(call("numflies"), '2')
        self.assertEquals(call("decr test"), '1')
        self.assertEquals(call("numflies"), '1')
        self.assertEquals(call('set test env test=1,test2=2'), 'ok')
        self.assertEquals(call('get test env'), 'env: test=1,test2=2')
